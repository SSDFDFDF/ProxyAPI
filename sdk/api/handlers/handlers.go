...
func requestExecutionMetadata(ctx context.Context, stickyHeaders []string) map[string]any {
	...
	// Sticky session: if no explicit pinnedAuthID, extract session hash from headers.
	if _, hasPinned := meta[coreexecutor.PinnedAuthMetadataKey]; !hasPinned && len(stickyHeaders) > 0 {
		if ctx != nil {
			if ginCtx, ok := ctx.Value("gin").(*gin.Context); ok && ginCtx != nil && ginCtx.Request != nil {
				if info := coreauth.ExtractSessionInfo(ginCtx.Request.Header, stickyHeaders); info != nil {
					// Use the authenticated API key as namespace to isolate sticky
					// bindings between different callers. Fall back to client IP.
					ns := ginCtx.GetString("apiKey")
					if ns == "" {
						ns = ginCtx.ClientIP()
					}
					hash := coreauth.GenerateStickySessionHash(ns, info.Source, info.ID)
					if hash != "" {
						meta[coreexecutor.StickySessionHashMetadataKey] = hash
					}
				}
			}
		}
...
func (h *BaseAPIHandler) GetContextWithCancel(handler interfaces.APIHandler, c *gin.Context, ctx context.Context) (context.Context, APIHandlerCancelFunc) {
	...
	return newCtx, func(params ...interface{}) {
		if h.Cfg.RequestLog && len(params) == 1 {
			if existingBytes := contextAPIResponseBytes(c); len(bytes.TrimSpace(existingBytes)) > 0 {
			...
			if len(payload) > 0 {
				if existingBytes := contextAPIResponseBytes(c); len(existingBytes) > 0 {
...
func appendAPIResponse(c *gin.Context, data []byte) {
	...
	if existing, exists := c.Get("API_RESPONSE"); exists {
		...
		case []byte:
			if len(value) > 0 {
				combined := make([]byte, 0, len(value)+len(data)+1)
...
func contextAPIResponseBytes(c *gin.Context) []byte {
	if c == nil {
		return nil
	}
...
func (h *BaseAPIHandler) ExecuteWithAuthManager(ctx context.Context, handlerType, modelName string, rawJSON []byte, alt string) ([]byte, http.Header, *interfaces.ErrorMessage) {
	...
	reqMeta := requestExecutionMetadata(ctx, h.AuthManager.StickyHeaders())
...
func (h *BaseAPIHandler) WriteErrorResponse(c *gin.Context, msg *interfaces.ErrorMessage) {
	...
	previous := bytes.Clone(contextAPIResponseBytes(c))
