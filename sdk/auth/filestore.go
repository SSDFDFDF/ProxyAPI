...
// SetRuntimeConfig updates the configuration used for auxiliary outbound requests
// performed while reading auth files, such as token refresh or project discovery.
func (s *FileTokenStore) SetRuntimeConfig(cfg *config.Config) {
	s.configLock.Lock()
...
func (s *FileTokenStore) readAuthFile(path, baseDir string) (*cliproxyauth.Auth, error) {
	...
	if provider == "antigravity" || provider == "gemini" {
		httpClient := s.auxHTTPClient(provider, metadata)
		projectID := ""
		...
		if projectID == "" {
			...
			if accessToken != "" {
				fetchedProjectID, errFetch := FetchAntigravityProjectID(context.Background(), accessToken, httpClient)
...
func (s *FileTokenStore) runtimeConfigSnapshot() *config.Config {
	s.configLock.RLock()
