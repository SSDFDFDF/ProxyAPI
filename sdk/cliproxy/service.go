...
type Service struct {
	...
	// lastStickyKey tracks the normalized sticky config to avoid unnecessary store recreation.
...
func (s *Service) applyStickyConfig(cfg *config.Config) {
	if s == nil || s.coreManager == nil || cfg == nil {
		return
	}
	stickyConf := cfg.Routing.StickySession
	if !stickyConf.Enabled {
		s.coreManager.SetStickyStore(nil)
		s.coreManager.SetStickyHeaders(nil)
		s.lastStickyKey = ""
		return
	}

	ttl, err := time.ParseDuration(stickyConf.TTL)
	if err != nil || ttl <= 0 {
		ttl = time.Hour
	}
	maxEntries := stickyConf.MaxEntries
	if maxEntries <= 0 {
		maxEntries = 100000
	}
	headers := stickyConf.Headers
	if len(headers) == 0 {
		headers = []string{"session_id", "conversation_id"}
	}

	// Only recreate the store when the capacity parameters actually change.
	newKey := fmt.Sprintf("enabled:%v:ttl:%v:max:%d", true, ttl, maxEntries)
	if newKey != s.lastStickyKey {
		s.coreManager.SetStickyStore(coreauth.NewStickySessionStore(ttl, maxEntries))
		s.lastStickyKey = newKey
	}
	s.coreManager.SetStickyHeaders(headers)
}
