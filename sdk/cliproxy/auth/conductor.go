...
type Manager struct {
	...
	// stickyStore holds session-to-credential affinity mappings.
	// Accessed atomically to avoid data races with config hot-reload.
	stickyStore atomic.Pointer[StickySessionStore]
	// stickyHeaders lists header names used to derive the sticky session key.
...
// SetStickyStore sets or clears the sticky session store.
// Pass nil to disable sticky session routing.
func (m *Manager) SetStickyStore(store *StickySessionStore) {
	m.stickyStore.Store(store)
}

// SetStickyHeaders sets the header names used to derive sticky session keys.
func (m *Manager) SetStickyHeaders(headers []string) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.stickyHeaders = headers
}

// StickyHeaders returns the configured sticky session header candidates.
func (m *Manager) StickyHeaders() []string {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.stickyHeaders
}

// StickyStore returns the sticky session store, or nil if disabled.
func (m *Manager) StickyStore() *StickySessionStore {
	return m.stickyStore.Load()
}
...
func stickySessionHashFromMetadata(meta map[string]any) string {
	if len(meta) == 0 {
		return ""
	}
	raw, ok := meta[cliproxyexecutor.StickySessionHashMetadataKey]
	if !ok || raw == nil {
		return ""
	}
...
func cloneMetadataWithPinnedAuth(meta map[string]any, authID string) map[string]any {
	cloned := make(map[string]any, len(meta)+1)
	for k, v := range meta {
		cloned[k] = v
	}
	cloned[cliproxyexecutor.PinnedAuthMetadataKey] = authID
	return cloned
}
...
func (m *Manager) pickNext(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	store := m.stickyStore.Load()
	explicitPinned := pinnedAuthIDFromMetadata(opts.Metadata)
	if explicitPinned != "" || store == nil {
		return m.pickNextCore(ctx, provider, model, opts, tried)
	}

	sessionHash := stickySessionHashFromMetadata(opts.Metadata)
	if sessionHash == "" {
		return m.pickNextCore(ctx, provider, model, opts, tried)
	}

	if stickyAuthID, ok := store.Get(sessionHash); ok {
		if _, alreadyTried := tried[stickyAuthID]; !alreadyTried {
			stickyOpts := opts
			stickyOpts.Metadata = cloneMetadataWithPinnedAuth(opts.Metadata, stickyAuthID)
			auth, exec, err := m.pickNextCore(ctx, provider, model, stickyOpts, tried)
			if err == nil && auth != nil {
				store.Refresh(sessionHash)
				return auth, exec, nil
			}
		}
		store.Delete(sessionHash)
	}

	auth, exec, err := m.pickNextCore(ctx, provider, model, opts, tried)
	if err == nil && auth != nil {
		store.Set(sessionHash, auth.ID)
	}
	return auth, exec, err
}

func (m *Manager) pickNextCore(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
...
func (m *Manager) pickNextMixed(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	...
	explicitPinned := pinnedAuthIDFromMetadata(opts.Metadata)
	if explicitPinned != "" || store == nil {
		return m.pickNextMixedCore(ctx, providers, model, opts, tried)
	}
	...
	if stickyAuthID, ok := store.Get(sessionHash); ok {
		if _, alreadyTried := tried[stickyAuthID]; !alreadyTried {
			stickyOpts := opts
			stickyOpts.Metadata = cloneMetadataWithPinnedAuth(opts.Metadata, stickyAuthID)
			auth, exec, provKey, err := m.pickNextMixedCore(ctx, providers, model, stickyOpts, tried)
			if err == nil && auth != nil {
				store.Refresh(sessionHash)
				return auth, exec, provKey, nil
			}
		}
		store.Delete(sessionHash)
	}

	auth, exec, provKey, err := m.pickNextMixedCore(ctx, providers, model, opts, tried)
	if err == nil && auth != nil {
		store.Set(sessionHash, auth.ID)
	}
	return auth, exec, provKey, err
}

func (m *Manager) pickNextMixedCore(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	if !m.useSchedulerFastPath() {
