...
type RemoteManagement struct {
	...
	// When false, background updates only run when panel-github-repository is explicitly configured.
	DisableAutoUpdatePanel bool `yaml:"disable-auto-update-panel"`
	// PanelGitHubRepository optionally points to the GitHub repository used to fetch the management panel asset.
...
// StickySessionConfig configures sticky session behavior for routing.
// When enabled, requests with matching session identifiers are routed
// to the same credential, preserving conversational context.
type StickySessionConfig struct {
	// Enabled activates sticky session routing.
	Enabled bool `yaml:"enabled,omitempty" json:"enabled,omitempty"`
	// TTL is the duration string (e.g. "1h", "30m") for session affinity lifetime.
	TTL string `yaml:"ttl,omitempty" json:"ttl,omitempty"`
	// MaxEntries is the maximum number of session mappings to keep in memory.
	MaxEntries int `yaml:"max-entries,omitempty" json:"max-entries,omitempty"`
	// Headers lists request header names used to derive the session identity.
...
type RoutingConfig struct {
	...
	// StickySession configures sticky session affinity for credential routing.
