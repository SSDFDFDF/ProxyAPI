...
func TestLoadConfigOptional_PanelGitHubRepository(t *testing.T) {
...
`)
		...
		if got := cfg.RemoteManagement.PanelGitHubRepository; got != "" {
			t.Fatalf("PanelGitHubRepository = %q, want empty", got)
		}
	})

	t.Run("configured value is trimmed", func(t *testing.T) {
		dir := t.TempDir()
		configPath := filepath.Join(dir, "config.yaml")
		configYAML := []byte(`
remote-management:
...
`)
		...
		if got := cfg.RemoteManagement.PanelGitHubRepository; got != "https://github.com/example/panel" {
			t.Fatalf("PanelGitHubRepository = %q, want %q", got, "https://github.com/example/panel")
		}
	})
}
