...
func DoLogin(cfg *config.Config, projectID string, options *LoginOptions) {
	...
	record, errLogin := authenticator.Login(ctx, effectiveCfg, loginOpts)
	if errLogin != nil {
	...
	httpClient, errClient := geminiAuth.GetAuthenticatedClient(ctx, storage, effectiveCfg, &gemini.WebLoginOptions{
		NoBrowser:    options.NoBrowser,
		CallbackPort: options.CallbackPort,
		Prompt:       callbackPrompt,
	})
	...
	if stableAccount != "" {
		...
		if errInherit := resin.InheritLease(ctx, cfg, loginIdentity.TempAccount, stableAccount); errInherit != nil {
			log.Warnf("Resin inherit-lease warning (non-fatal): %v", errInherit)
		}
	}
	...
	if setter, okSetter := store.(interface{ SetRuntimeConfig(*config.Config) }); okSetter && cfg != nil {
		setter.SetRuntimeConfig(cfg)
	}
