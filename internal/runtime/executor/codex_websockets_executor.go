...
func (e *CodexWebsocketsExecutor) dialCodexWebsocket(ctx context.Context, auth *cliproxyauth.Auth, wsURL string, headers http.Header) (*websocket.Conn, *http.Response, error) {
	var resinActive bool
	wsURL, headers, resinActive = routeCodexWebsocketThroughResin(e.cfg, auth, wsURL, headers)
	var dialer *websocket.Dialer
	if resinActive {
		// Resin already rewrote the URL to its own endpoint; skip the original
		// proxy so we don't double-proxy.
...
func routeCodexWebsocketThroughResin(cfg *config.Config, auth *cliproxyauth.Auth, wsURL string, headers http.Header) (string, http.Header, bool) {
	...
	targetURL, err := url.Parse(strings.TrimSpace(wsURL))
	if err != nil {
		log.Errorf("codex websockets executor: parse websocket url failed: %v", err)
		return wsURL, headers, false
	}

	rewritten, err := parsedResin.ReverseURL(targetURL)
	if err != nil {
		log.Errorf("codex websockets executor: resin websocket rewrite failed: %v", err)
		return wsURL, headers, false
	}
	...
	clonedHeaders.Set(resin.AccountHeader, account)
	return rewritten.String(), clonedHeaders, true
}
