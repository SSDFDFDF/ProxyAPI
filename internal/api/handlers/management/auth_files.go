...
func (h *Handler) tokenStoreWithBaseDir() coreauth.Store {
	...
	if h.cfg != nil {
		...
		if cfgSetter, ok := store.(interface{ SetRuntimeConfig(*config.Config) }); ok {
			cfgSetter.SetRuntimeConfig(h.cfg)
		}
	}
	return store
}
...
func (h *Handler) resinLoginConfig(provider string) (*config.Config, resin.Identity, error) {
...
func (h *Handler) promoteResinLease(ctx context.Context, record *coreauth.Auth, identity resin.Identity) error {
	if record == nil {
		return nil
	}
	...
	return resin.InheritLease(ctx, h.cfg, identity.TempAccount, stableAccount)
}
...
func (h *Handler) RequestAnthropicToken(c *gin.Context) {
	...
	effectiveCfg, loginIdentity, errResinCfg := h.resinLoginConfig("claude")
	if errResinCfg != nil {
		log.Errorf("Failed to initialize Claude authentication: %v", errResinCfg)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to initialize authentication"})
		return
	}

	// Initialize Claude auth service
	anthropicAuth := claude.NewClaudeAuth(effectiveCfg)
	...
	go func() {
		...
		if errResin := h.promoteResinLease(ctx, record, loginIdentity); errResin != nil {
			log.Warnf("Resin inherit-lease warning (non-fatal): %v", errResin)
		}
...
func (h *Handler) RequestGeminiCLIToken(c *gin.Context) {
	ctx := context.Background()
	ctx = PopulateAuthContext(ctx, c)
	effectiveCfg, loginIdentity, errResinCfg := h.resinLoginConfig("gemini")
	if errResinCfg != nil {
		log.Errorf("Failed to initialize Google authentication: %v", errResinCfg)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to initialize authentication"})
		return
	}
...
func (h *Handler) RequestCodexToken(c *gin.Context) {
	...
	effectiveCfg, loginIdentity, errResinCfg := h.resinLoginConfig("codex")
	if errResinCfg != nil {
		log.Errorf("Failed to initialize Codex authentication: %v", errResinCfg)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to initialize authentication"})
		return
	}

	// Initialize Codex auth service
	openaiAuth := codex.NewCodexAuth(effectiveCfg)
...
func (h *Handler) RequestAntigravityToken(c *gin.Context) {
	...
	effectiveCfg, loginIdentity, errResinCfg := h.resinLoginConfig("antigravity")
	if errResinCfg != nil {
		log.Errorf("Failed to initialize Antigravity authentication: %v", errResinCfg)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to initialize authentication"})
		return
	}
...
func (h *Handler) RequestQwenToken(c *gin.Context) {
	...
	state := fmt.Sprintf("gem-%d", time.Now().UnixNano())
	effectiveCfg, loginIdentity, errResinCfg := h.resinLoginConfig("qwen")
	if errResinCfg != nil {
		log.Errorf("Failed to initialize Qwen authentication: %v", errResinCfg)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to initialize authentication"})
		return
	}
	// Initialize Qwen auth service
	qwenAuth := qwen.NewQwenAuth(effectiveCfg)
...
func (h *Handler) RequestKimiToken(c *gin.Context) {
	...
	effectiveCfg, loginIdentity, errResinCfg := h.resinLoginConfig("kimi")
	if errResinCfg != nil {
		log.Errorf("Failed to initialize Kimi authentication: %v", errResinCfg)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to initialize authentication"})
		return
	}
	// Initialize Kimi auth service
	kimiAuth := kimi.NewKimiAuth(effectiveCfg)
...
func (h *Handler) RequestIFlowToken(c *gin.Context) {
	...
	effectiveCfg, loginIdentity, errResinCfg := h.resinLoginConfig("iflow")
	if errResinCfg != nil {
		log.Errorf("Failed to initialize iFlow authentication: %v", errResinCfg)
		c.JSON(http.StatusInternalServerError, gin.H{"status": "error", "error": "failed to initialize authentication"})
		return
	}
	authSvc := iflowauth.NewIFlowAuth(effectiveCfg)
