...
func TestExtractAPIResponseSupportsBufferValue(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)

	wrapper := &ResponseWriterWrapper{}
	c.Set("API_RESPONSE", bytes.NewBufferString("buffered-response"))

	body := wrapper.extractAPIResponse(c)
	if string(body) != "buffered-response" {
		t.Fatalf("api response = %q, want %q", string(body), "buffered-response")
	}
}
