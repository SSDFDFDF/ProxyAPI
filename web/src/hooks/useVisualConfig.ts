    ...
    try {
      ...
      const newValues: VisualConfigValues = {
        ...
        stickySessionEnabled: (() => {
          const stickySession = routing ? asRecord(routing['sticky-session']) : undefined;
          return Boolean(stickySession?.enabled ?? false);
        })(),
        stickySessionTTL: (() => {
          const stickySession = routing ? asRecord(routing['sticky-session']) : undefined;
          return String(stickySession?.ttl ?? '1h');
        })(),
        stickySessionMaxEntries: (() => {
          const stickySession = routing ? asRecord(routing['sticky-session']) : undefined;
          return String(stickySession?.['max-entries'] ?? '100000');
        })(),
        stickySessionHeaders: (() => {
          const stickySession = routing ? asRecord(routing['sticky-session']) : undefined;
          return Array.isArray(stickySession?.headers)
            ? (stickySession.headers as string[]).join(', ')
            : String(stickySession?.headers ?? 'session_id, conversation_id');
        })(),
    ...
    (currentYaml: string): string => {
      try {
        ...
        if (
          ...
          if (values.stickySessionEnabled) {
            ensureMapInDoc(doc, ['routing', 'sticky-session']);
            doc.setIn(['routing', 'sticky-session', 'enabled'], true);
            const ttl = values.stickySessionTTL.trim();
            if (ttl && ttl !== '1h') {
              doc.setIn(['routing', 'sticky-session', 'ttl'], ttl);
            } else {
              if (docHas(doc, ['routing', 'sticky-session', 'ttl'])) {
                doc.deleteIn(['routing', 'sticky-session', 'ttl']);
              }
            }
            const maxEntries = parseInt(values.stickySessionMaxEntries, 10);
            if (!isNaN(maxEntries) && maxEntries !== 100000) {
              doc.setIn(['routing', 'sticky-session', 'max-entries'], maxEntries);
            } else {
              if (docHas(doc, ['routing', 'sticky-session', 'max-entries'])) {
                doc.deleteIn(['routing', 'sticky-session', 'max-entries']);
              }
            }
            const headers = values.stickySessionHeaders
              .split(',')
              .map((h: string) => h.trim())
              .filter(Boolean);
            const defaultHeaders = ['session_id', 'conversation_id'];
            const isDefault =
              headers.length === defaultHeaders.length &&
              headers.every((h: string, i: number) => h === defaultHeaders[i]);
            if (!isDefault) {
              doc.setIn(['routing', 'sticky-session', 'headers'], doc.createNode(headers));
            } else {
              if (docHas(doc, ['routing', 'sticky-session', 'headers'])) {
                doc.deleteIn(['routing', 'sticky-session', 'headers']);
              }
            }
            deleteIfMapEmpty(doc, ['routing', 'sticky-session']);
          } else {
            if (docHas(doc, ['routing', 'sticky-session'])) {
              doc.deleteIn(['routing', 'sticky-session']);
            }
          }
